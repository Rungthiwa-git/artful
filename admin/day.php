...
<form method="post" action="" class="select" style="margin-top: 20px;">
    ...
    $query = "
    SELECT o.order_price, SUM(o.order_price) AS total, DATE_FORMAT(o.order_timestamp, '%d-%M-%Y') AS datesave
    FROM orders o
    ...
    GROUP BY DATE_FORMAT(o.order_timestamp, '%d-%M-%Y') 
    ORDER BY DATE_FORMAT(o.order_timestamp, '%Y-%m-%d') DESC
    LIMIT 7";
    $query1 = "
    SELECT DATE_FORMAT(o.order_timestamp, '%d-%M-%Y') AS datesave, COUNT(o.order_id) AS order_count
    FROM orders o
    JOIN picture p ON o.picture_id = p.PICTURE_ID
    JOIN user u ON o.USER_ID = u.USER_ID 
    WHERE 1 $categoryFilterCondition   $pictureTypeFilterCondition $sexFilterCondition $statusFilterCondition  $jobFilterCondition $ageFilterCondition $religionFilterCondition $countryFilterCondition
        ...
        <script>
            ...
            var myChart1 = new Chart(ctx1, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Income report separated by day (USD)',
                        data: [<?php echo $total; ?>],
                        backgroundColor: [
                            'rgba(255, 99, 132,0.8)',
                            'rgba(255, 99, 132,0.5)',
    ...
    <hr>
    <h3 align="center">Report order by day </h3>
    <p align="center">
        <!-- Second Chart -->
